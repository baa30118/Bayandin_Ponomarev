...
def load(id):  # load(car_id)
               # ---> ((id, model, year, color, plate_number, car_type), (id, id_cars, date, damage_description))

    cur.execute('SELECT id, model, "year", color, plate_number, car_type FROM bayandin_ponomarev.cars WHERE id = %s;',
                (id,))

    car_info = cur.fetchall()

    cur.execute(
        'SELECT id_accidence, id_cars, plate_num_cars, "date", damage_description FROM bayandin_ponomarev.accidents WHERE id_cars = %s;',
        (id,))
